...
public class Functions
{
    ...
    public float Identity(float x, bool derivative, LossDelegate loss, params float[] a)
    {
        ...
        else return (loss != null) ? loss(a[a.Length - 1], x, (a.Length - 1) / 2, true) : 1 * 1;
    }
    ...
    public float ReLU(float x, bool derivative, LossDelegate loss, params float[] a)
    {
        ...
        else { if (x > 0) return (loss != null) ? loss(a[a.Length - 1], x, (a.Length - 1) / 2, true) : 1 * 1; else return 0; }
    }
    ...
    public float ELU(float x, bool derivative, LossDelegate loss, params float[] a)
    {
        ...
        else { float X =Mathf.Log(x/A); if (x >= 0) return 1; else return (loss != null) ? loss(a[a.Length - 1], x, (a.Length - 1) / 2, true) : 1 * A * Mathf.Exp(X); }
    }
    public float Sigmoid(float x, bool derivative, LossDelegate loss, params float[] a)
    {
        float X = 1 / (1 + Mathf.Exp(-x));
        if (!derivative) return X;
        else return (loss != null) ? loss(a[a.Length - 1], x, (a.Length - 1) / 2, true) : 1 * x * (1 - x);
    }
    public float Softplus(float x, bool derivative, LossDelegate loss, params float[] a)
    {
        ...
        else { float X =Mathf.Log(Mathf.Exp(x)-1); return (loss != null) ? loss(a[a.Length - 1], x, (a.Length - 1) / 2, true) : 1 * 1 / (1 + Mathf.Exp(-X)); }
    }
    public float TanH(float x, bool derivative, LossDelegate loss, params float[] a)
    {
        float X = (2 / (1 + Mathf.Exp(-2 * x))) - 1;
        if (!derivative) return X;
        else return (loss != null) ? loss(a[a.Length - 1], x, (a.Length - 1) / 2, true) : 1 * 1 - (x * x);
    }
    ...
    public float Gaussian(float x, bool derivative, LossDelegate loss, params float[] a)
    {
        ...
        else { float X =Mathf.Sqrt(-Mathf.Log(x)); return (loss != null) ? loss(a[a.Length - 1], x, (a.Length - 1) / 2, true) : 1 * -2 * X * Mathf.Exp(-X * X); }
    }
